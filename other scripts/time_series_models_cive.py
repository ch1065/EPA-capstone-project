...
names = sub.columns.drop(['County Name', 'State Name', 'Date Local', 'Time Local', 'Site Num', 'Exceedance'])
...
sub = sub.drop(['County Name', 'State Name', 'Date Local', 'Time Local', 'Site Num', \
    'Exceedance'], axis = 1).dropna().reset_index(drop = True)

#sub['Ozone (Parts per million)'] = sub['Ozone (Parts per million)'] - sub['Ozone (Parts per million)'].rolling(window = 720).mean()

df_decomposed_add = seasonal_decompose(sub['Ozone (Parts per million)'], model='additive', period = 24)
...
hwes = ExponentialSmoothing(train['Ozone (Parts per million)'])
