...
dates = pd.DataFrame({'date_time' : pd.date_range(start = '2020-01-01', end = '2022-01-01', freq = 'H')}).set_index('date_time')
...
names = sub.columns.drop(['County Name', 'State Name', 'Date Local', 'Time Local', 'Site Num', 'Exceedance_mc', 'Exceedance_bc'])
...
sub = sub.drop(['County Name', 'State Name', 'Date Local', 'Time Local', 'Site Num', \
    'Exceedance_mc', 'Exceedance_bc'], axis = 1).dropna()
...
bronx = bronx.set_index('date_time', drop = True).drop(['County Name', 'State Name', 'Date Local', 'Time Local', 'Site Num', 'Exceedance_mc', 'Exceedance_bc'], axis = 1)

merged = sub.merge(bronx, on = 'date_time')


# removing nas
sub = merged.dropna().reset_index(drop = True).drop(['Latitude_y', 'Longitude_y'], axis = 1)


#sub['Ozone (Parts per million)'] = sub['Ozone (Parts per million)'] - sub['Ozone (Parts per million)'].rolling(window = 720).mean()

df_decomposed_add = seasonal_decompose(sub['Ozone (Parts per million)_x'], model='additive', period = 24)
...
ad_test(df_og['Ozone (Parts per million)_x'])


#step_fit = auto_arima(df_og['Ozone (Parts per million)'], trace = True, suppress_warnings = True, start_p = 5, start_q = 1, max_p = 12, max_q = 3,\
#    d = None, error_action='ignore', stepwise = True, seasonal = True, m = 24, D = 1)
...
y_test = test['Ozone (Parts per million)_x']

hwes = ExponentialSmoothing(train['Ozone (Parts per million)_x'], seasonal_periods=24, trend='add', seasonal='mul')
fitted = hwes.fit(optimized=True, use_brute=True)

preds_hwes = fitted.forecast(steps=100)
...
arima = ARIMA(train['Ozone (Parts per million)_x'], order = (1, 1, 0), seasonal_order=(2, 0, 1, 24))
arima_r = arima.fit()
preds_arima = arima_r.predict(start = len(train), end = (len(train) + len(test) - 1), type = 'levels')
arima_rmse = math.sqrt(mean_squared_error(test['Ozone (Parts per million)_x'], preds_arima))
