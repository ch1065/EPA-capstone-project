...
f0 = pd.read_csv(tmp[0]).to_csv('data/final_1.2.csv', index = False)
os.remove(tmp[0])
# looping through the files 
for i in range(1, f_len):
    # open the final csv file so we can add to it
    final = pd.read_csv('data/final_1.2.csv')
    ...
    m.to_csv('data/final_1.2.csv',  index = False)
    os.remove(tmp[i])

final = pd.read_csv('data/final_1.2.csv')

# changing ozone values to levels--making it a classification problem
final['Exceedance'] = np.nan

final.loc[(final['Ozone (Parts per million)'] > 0.300), 'Exceedance'] = 5
final.loc[(final['Ozone (Parts per million)'] > 0.200) & (final['Ozone (Parts per million)'] <= 0.300), 'Exceedance'] = 4
final.loc[(final['Ozone (Parts per million)'] > 0.150) & (final['Ozone (Parts per million)'] <= 0.200), 'Exceedance'] = 3
final.loc[(final['Ozone (Parts per million)'] > 0.100) & (final['Ozone (Parts per million)'] <= 0.150), 'Exceedance'] = 2
final.loc[(final['Ozone (Parts per million)'] > 0.05) & (final['Ozone (Parts per million)'] <= 0.1), 'Exceedance'] = 1
final.loc[(final['Ozone (Parts per million)'] <= 0.05), 'Exceedance'] = 0

final.to_csv('data/final_1.2.csv', index = False) 
