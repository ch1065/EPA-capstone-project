...
f0 = pd.read_csv(tmp[0]).to_csv('data/final_1.1.csv', index = False)
os.remove(tmp[0])
# looping through the files 
for i in range(1, f_len):
    # open the final csv file so we can add to it
    final = pd.read_csv('data/final_1.1.csv')
    ...
    m.to_csv('data/final_1.1.csv',  index = False)
    os.remove(tmp[i])
...
final.loc[(final['Ozone (Parts per million)'] > 0.05), 'Exceedance'] = 1
final.loc[(final['Ozone (Parts per million)'] <= 0.05), 'Exceedance'] = 0

final.to_csv('data/final_1.1.csv', index = False) 
