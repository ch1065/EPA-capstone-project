...
for i in range(0, len(files), 3):
    ...
    cat = pd.concat([og_19, og_20, og_21], ignore_index = True, axis = 0)
...
f0 = pd.read_csv(tmp[0]).to_csv('data/final_1.3.csv', index = False)
os.remove(tmp[0])
# looping through the files 
for i in range(1, f_len):
    # open the final csv file so we can add to it
    final = pd.read_csv('data/final_1.3.csv')
    ...
    m.to_csv('data/final_1.3.csv',  index = False)
    os.remove(tmp[i])
...
final.to_csv('data/final_1.3.csv', index = False) 