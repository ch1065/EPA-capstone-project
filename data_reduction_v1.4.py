...
def tidy(og):
    ...
    #sub_og = og.loc[((og['County Name'] == 'Fairfield') & (og['Site Num'] == 9003)) | ((og['County Name'] == 'Bronx') & (og['Site Num'] == 110)) | \
    #((og['County Name'] == 'District of Columbia') & (og['Site Num'] == 41) | ((og['County Name'] == 'Philadelphia') & (og['Site Num'] == 48)))]
...
for i in range(0, len(files), 4):
    ...
    # opening the second file (2021 of a given parameter)
    og = pd.read_csv(files[i + 2])
    # passing dataframe to the tidy function
    og_20 = tidy(og)

    del og

    og = pd.read_csv(files[i + 3])
    og_21 = tidy(og)
    # concatenating the 2020 and 2021 dataframes together
    cat = pd.concat([og_18, og_19, og_20, og_21], ignore_index = True, axis = 0)
...
f0 = pd.read_csv(tmp[0]).to_csv('data/final_1.4.csv', index = False)
os.remove(tmp[0])
# looping through the files 
for i in range(1, f_len):
    # open the final csv file so we can add to it
    final = pd.read_csv('data/final_1.4.csv')
    ...
    m.to_csv('data/final_1.4.csv',  index = False)
    os.remove(tmp[i])

final = pd.read_csv('data/final_1.4.csv')

# changing ozone values to levels--making it a multiclassification problem
final['Exceedance_mc'] = np.nan

final.loc[(final['Ozone (Parts per million)'] > 0.300), 'Exceedance_mc'] = 5
final.loc[(final['Ozone (Parts per million)'] > 0.200) & (final['Ozone (Parts per million)'] <= 0.300), 'Exceedance_mc'] = 4
final.loc[(final['Ozone (Parts per million)'] > 0.150) & (final['Ozone (Parts per million)'] <= 0.200), 'Exceedance_mc'] = 3
final.loc[(final['Ozone (Parts per million)'] > 0.100) & (final['Ozone (Parts per million)'] <= 0.150), 'Exceedance_mc'] = 2
final.loc[(final['Ozone (Parts per million)'] > 0.05) & (final['Ozone (Parts per million)'] <= 0.1), 'Exceedance_mc'] = 1
final.loc[(final['Ozone (Parts per million)'] <= 0.05), 'Exceedance_mc'] = 0

# changing ozone values to levels--making it a binary classification problem
final['Exceedance_bc'] = np.nan

final.loc[(final['Ozone (Parts per million)'] > 0.05), 'Exceedance_bc'] = 1
final.loc[(final['Ozone (Parts per million)'] <= 0.05), 'Exceedance_bc'] = 0

final.to_csv('data/final_1.4.csv', index = False) 